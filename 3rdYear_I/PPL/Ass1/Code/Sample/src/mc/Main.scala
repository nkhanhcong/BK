...
object Main extends  Timed {
  ...
  def main(args: Array[String]): Unit = {
    ...
    for (i <- start to end) {
      ...
      val (source,dest) = (new ANTLRFileStream(s"$indir$sepa$i.txt"),new PrintWriter(new File(s"$outdir$sepa$i.txt"))) //dung trong linux     
      ...
      try 
      {
        timeoutAfter(1000)  //thoi gian chay la 0.5s
        {
            option match {
              ...
              case `parser` => TestParser.test(source,dest)
